...
```bash
# General research
gemini -p "Research best practices for implementing OAuth2 in TypeScript.
Include:
- Recommended libraries (compare next-auth vs lucia vs arctic vs others)
- Security considerations
- Common pitfalls to avoid
- Example implementations" 2>/dev/null

# Framework-specific research
gemini -p "Research Next.js authentication patterns in 2025.
Focus on:
...
# Compare libraries
gemini -p "Compare these TypeScript HTTP clients in 2025:
...
```bash
# Framework migration
gemini -p "Analyze our codebase for Express to Hono migration:
- Identify all Express-specific patterns used
- Map to Hono equivalents
- Estimate migration complexity per module
- Suggest migration order" --include-directories src 2>/dev/null

# Runtime upgrade
gemini -p "Research breaking changes from Node.js 20 to 22.
