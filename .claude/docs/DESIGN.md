...
Claude Code Orchestra is a multi-agent collaboration framework. Claude Code (200K context) is the orchestrator, with Gemini CLI (1M context) for codebase analysis, research, and multimodal reading, and subagents (Opus) for code implementation.
...
│  │ Implementer A/B/C     │  │ Gemini consultation   │             │
│  │ Security/Quality Rev. │  │                       │             │
│  └──────────────────────┘  └──────────────────────┘             │
│                                                                   │
│  External CLIs:                                                   │
...
| general-purpose（Opus） | 実装 | コード実装、ファイル操作 |
...
| Subagents | Isolated tasks returning results | External research, implementation |
...
|---------|------|---------|-------|
| Gemini CLI | Codebase analysis, research, multimodal reading | gemini-3-pro | 1M context: codebase analysis + external research + PDF/video/audio/image extraction |

### Trigger Classification

| Type | Action | Who Initiates |
|------|--------|--------------|
| **Automatic** | Gemini CLI call | Claude (judgment flow) |
| **Automatic** | Subagent delegation (Task tool) | Claude (output size / context) |
| **Automatic** | Multimodal file delegation | Claude (file extension detection) |
| **Manual** | `/startproject` | User (slash command) |
| **Manual** | `/team-implement` | User (slash command) |
| **Manual** | `/team-review` | User (slash command) |
...
| 2026-02-17 | Role clarification: Gemini → multimodal + codebase analysis + research |
